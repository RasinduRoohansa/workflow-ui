...
import {ROUTES} from './sidebar-admin-routes.config';
...
export class SidebarAdminComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit() {
        this.httpService
            .httpGet(this.appConstant.baseURL + '/' + this.appConstant.findMenuByUsername + '/' + this.store.getData(AUTH.username))
            .subscribe(res => {
                this.menus = res;
    ...
    logout() {
        ...
        window.location.href = '#/' + this.appConstant.loginURL;
    }

    lockMe() {
        this.store.removeData(AUTH.token);
        this.route.navigate([this.appConstant.lockURL]);
    }
}
